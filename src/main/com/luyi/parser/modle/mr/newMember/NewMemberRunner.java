...
public class NewMemberRunner implements   Tool{
    ...
    public int run(String[] args) throws Exception {
        ...
        if (job.waitForCompletion(true)) {
            addTotalNewMember(job);
            addBrowserTotalNewMember(job);
            return 0;
        }else {
            return 1;
        }
    }
    public  void addTotalNewMember(Job job) {
            ...
            if (thisDid > 0){
                ...
                while (rs.next()){
                    ...
                    if (info.containsKey(platform)){
                        newMembers += info.get(platform);
                    }
                    info.put(platform,newMembers);
                }
            }

            for (Map.Entry<String,Integer> en : info.entrySet()){
                ps = conn.prepareStatement("insert into stats_user(date_dimension_id ,platform_dimension_id,total_members,created) " +
                        "value (?,?,?,?) on duplicate key update total_members = ?");
                ps.setInt(1,thisDid);
    ...
    public  void addBrowserTotalNewMember(Job job) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ...
            if (lastDid > 0){
                ...
                while (rs.next()){
                    ...
                    info.put(strarr,rs.getInt("total_members"));
                }
            }
            if (thisDid > 0){
                ...
                while (rs.next()){
                    ...
                    if (info.containsKey(strarr)){
                        newMembers += info.get(strarr);
                    }
                    info.put(strarr,newMembers);
                }
            }

            for (Map.Entry<String[],Integer> en : info.entrySet()){
                ps = conn.prepareStatement("insert into stats_device_browser(date_dimension_id ,platform_dimension_id,browser_dimension_id,total_members,created) " +
                        "value (?,?,?,?,?) on duplicate key update total_members = ?");
                ps.setInt(1,thisDid);
